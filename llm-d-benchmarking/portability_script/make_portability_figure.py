...
rows = []
for label, params_b, hw, tp, T_B_measured in portability.rows:
    if not label.startswith('Qwen3-32B'):
        continue
    T_B = T_B_measured if T_B_measured is not None else portability.estimate_T_B(params_b, hw, tp, B)
    short = label.replace('Qwen3-32B / ', '').replace('(measured)', '(anchored)')
    rows.append((short, B / T_B, T_B_measured is not None))
...
dashes = [(4, 2), (6, 2, 1, 2), (), (2, 2), (5, 2), (1, 1.5), (7, 3)]

fig, ax = plt.subplots(figsize=(10.4, 6.6), dpi=100)

t = [5, 30]
for (label, r_peak, anchored), color, dash in zip(rows, colors, dashes):
    ax.plot(t, [r_peak * x / 1000 for x in t],
            color=color,
            linewidth=2.75 if anchored else 1.75,
            dashes=dash if not anchored else (),
            solid_capstyle='round',
            label=label)

# Our deployment: the anchored H100 line at T_max = 14s
tau_k = portability.tau_sat(0.40, T_MAX, B)[1] / 1000
ax.plot([T_MAX], [tau_k], marker='*', markersize=17, color='#1baf7a',
